...
categoryNotifications.subscribe = function (uid, cid, callback) {
	db.sortedSetAdd('cid:' + cid + ':subscribed:uids', Date.now(), uid, function(err) {
		if (err) {
			callback(err);
			return;
		}
		db.sortedSetAdd('group:cid:' + cid + ':privileges:find:members', Date.now(), uid, callback);
	});
};

categoryNotifications.unsubscribe = function (uid, cid, callback) {
	db.sortedSetRemove('cid:' + cid + ':subscribed:uids', uid, function(err) {
		if (err) {
			callback(err);
			return;
		}
		db.sortedSetRemove('group:cid:' + cid + ':privileges:find:members', uid, callback);
	});
};
...
function getChildIDs(cid, resolve) {
