...
categoryNotifications.subscribe = function (uid, cid, callback) {
	db.sortedSetAdd('cid:' + cid + ':subscribed:uids', Date.now(), uid, function (err, data) {
		if (err) {
			callback(err, null);
			return;
		} else {
			getChildIDs(cid, function (err, cids) {
				cids.forEach(function (current) {
					db.sortedSetAdd('cid:' + current + ':subscribed:uids', Date.now(), uid);
				});
				callback(null, data);
			});
		}
